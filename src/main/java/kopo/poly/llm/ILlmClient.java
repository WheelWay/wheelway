...
/**
 * 사람이 한 말에서 <b>목적지 이름</b>과 <b>어떻게 갈 것인지</b>를 뽑는다.
 *
...
public interface ILlmClient {
     ...
     * 어떻게 갈 것인가.
     *
     * <h3>★ {@link #UNKNOWN} 이 정상 응답이다</h3>
     * {@code 청주시청으로 가주세요} 만으로는 걸어갈지 버스를 탈지 알 수 없다. 그때
     * 도보로 단정하면 <b>버스로 갈 거리를 걸어가라고 안내하게 된다.</b> 모르면 모른다고
     * 답하고, 되묻는 것은 화면이 한다({@code Answer.needMode}).
     *
     * <p>이것도 {@code destination} 과 <b>같은 호출에서</b> 온다. 수단을 따로 물으면
     * 호출이 두 배가 되고 RPM 15 에 그만큼 빨리 걸린다 — 스키마에 필드 하나를 더하는
     * 것으로 끝나는 일이라 나눌 이유가 없다.
     */
    enum Mode {
        /** 걸어서(휠체어로) 간다. */
        WALK,
        ...
        /** 택시(장애인콜택시)를 쓴다. */
        TAXI,
        /** 말만으로는 알 수 없다. <b>흔한 값이다</b> — 사람은 대개 수단을 말하지 않는다. */
        UNKNOWN
    }
     ...
     * @param mode        어떻게 갈 것인가. <b>말에 드러날 때만 정한다</b> — 짐작하지 않는다
     */
    record Destination(String destination, boolean matched, Mode mode) {
    }

    /**
     * 말 한 마디에서 목적지와 <b>이동수단</b>을 뽑는다.
     *
